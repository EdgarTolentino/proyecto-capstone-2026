...
export function AppShell({ children, catalogos, estado, sesion }: AppShellProps) {
  const obra = catalogos?.obras?.[0]?.nombre ?? "Edificio Costanera Sur";
    ...
    <div className="app-shell">
      ...
      <section className="workspace">
        <header className="global-header">
          ...
          <div className="user-name">{sesion?.nombre ?? "Cargando sesión…"}</div>
        </header>
        {children}
      </section>
    </div>
